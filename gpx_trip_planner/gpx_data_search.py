...
def search_nominatim(search_terms, scope=None, bbox=None):
	...
	print("URL:", url)
	http_resp = urlopen(url, timeout=10)
	resp_text = str(http_resp.read())
	#print(resp_text)
	...
	for match in tree.findall("place"):
		...
		if match_class == "boundary":
			match_obj.polygonpoints = [pykarta.geometry.Point(float(i[1]), float(i[0])) for i in json.loads(match.get('polygonpoints'))]
...
class SearchMatches(object):
	def __init__(self, map_obj):
		self.datastore = Gtk.ListStore(GObject.TYPE_PYOBJECT, GObject.TYPE_STRING, GdkPixbuf.Pixbuf, GObject.TYPE_STRING)
