...
class GpxFancyTreeView(GpxSimpleTreeView):
	...
	def show_pixmap_cb(self, column, cell_renderer, tree_model, iter, data):
		python_obj = tree_model.get_value(iter, 0)
		try:
	...
	def drag_motion_cb(self, treeview, context, x, y, etime):
		...
		if len(source_path) < 3 and (
				(len(source_path) == len(dest_path) and (dest_relative == Gtk.TreeViewDropPosition.BEFORE or dest_relative == Gtk.TreeViewDropPosition.AFTER))
					or
				((len(source_path) - len(dest_path)) == 1 and (dest_relative == Gtk.TreeViewDropPosition.INTO_OR_BEFORE or dest_relative == Gtk.TreeViewDropPosition.INTO_OR_AFTER))
				):
			print("  Approved!")
...
class GpxPicker(object):
	def __init__(self, builder, stem, data_from, data, point_depth, syncer):
		...
		if self.add_rp_button:
			...
			# FIXPORT: exception due to wrong child type
			#syncer.add(self.add_rp_list.get_child())
	...
	def add_rp_button_cb(self, widget):
		route_name = self.add_rp_list.get_child().get_text()
...
class GpxEditMenu(object):
	def __init__(self, builder, data, clipboard, search_cb, gpsr, ui, map):
		...
		for layer_name, layer_data in list(self.layer_data.items()):
	...
	def on_paste(self, widget):
		print("Edit->Paste")
		pasted_text = str(self.clipboard.wait_for_text())
		...
		# Try to parse pasted text as a coordinate
...
class GpxGUI(object):
	...
	def __init__(self, profile_dir):
		...
		for size in (16, 32, 48, 64, 128):
			icon_list.append(GdkPixbuf.Pixbuf.new_from_file(os.path.join(sys.path[0], "images", "app_icon-%dx%d.png" % (size, size))))
		self.main_window.set_default_icon_list(icon_list)

		#---------------------------------------------------------
		# Enlarge on large screens
		#---------------------------------------------------------
		if Gdk.Screen.height() >= 1024:
			self.main_window.set_default_size(1400, 900)
		...
		# FIXPORT: why is the tile source set to None?
		#self.map = MapWidget(tile_source=None, debug_level=1)
		self.map = MapWidget(debug_level=1)
		self.builder.get_object("MapVbox").pack_end(self.map, True, True, 0)
	...
	def set_tool(self, tool):
		...
		# FIXPORT: messes up root cursor
		if self.tool == "tool_adjust":
			self.map.set_cursor(Gdk.CursorType.FLEUR)
		elif self.tool == "tool_draw":
			self.map.set_cursor(Gdk.CursorType.PENCIL)
		elif self.tool == "tool_delete":
			self.map.set_cursor(Gdk.CursorType.X_CURSOR)
		else:
			self.map.set_cursor(None)

		# Inform the active layer of the tool change.
		try:
		...
		for layer in layers:
			if layer:
				...
				if layer.default:
					...
					self.on_base_layer_change(menuitem, layer)	# FIXME: had to add this
			else:	# separator
	...
	def on_map_menu(self, button, gdkevent):
		...
		def get_popup_pos(menu, x, y, button):
			...
			y += allocation.height
			return (x, y, True)	
		# FIXME: See https://stackoverflow.com/questions/58800674/is-gtk-window-set-attached-to-really-the-correct-way-to-position-a-popup-relat
		self.map_menu.popup(None, None, get_popup_pos, button, gdkevent.button, gdkevent.time)

	# Called whenever the user changes the base map layer selection in the menu
	...
	# Called whenever the user toggles an overlay layer on or off in the menu
	...
	def choose_file(self, title="Choose File", action='open', filetypes=[]):
		...
		chooser = Gtk.FileChooserDialog(
			title=title,
			action=action,
			buttons=(Gtk.STOCK_CANCEL,Gtk.ResponseType.CANCEL,button,Gtk.ResponseType.OK)
			)
	...
	def on_view_crop_lines(self, button):
		if button.get_active():
			...
			width, height, margin = list(map(int, button_name.split(':')[1:]))
			if width:
